...
func (k *kataAgent) startSandbox(sandbox *Sandbox) error {
	...
	if err = k.check(); err != nil {
		k.Logger().WithError(err).Debug("check ttrpc server failed!")
		return err
	}
...
func (k *kataAgent) connect() error {
	...
	// sleep for some time before connect,
	// when vm is booting and we connect to it, this crashes fc
